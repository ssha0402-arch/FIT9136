...
for order in queue: 
    ...
    temp_stock = part_stock.copy()  # set a temp list
    consed_num = 0 
    i = 0                           # set counter
    while i < len(model_req):                           # try to constrct full order
        check = 1
        if temp_stock[i] >= model_req[i] * model_num:
            check = 1
            i += 1
        else:
            check = 0
    ...
    if check == 1:
        ...
        while try_num > 0:
            ...
            while i < len(model_req):
                if temp_stock[i] >= model_req[i] * try_num:
                    check = 1
                    i += 1
                else:
                    check = 0
    ...
    if consed_num >= 1:
        i = 0
        while i < len(model_req):
            temp_stock[i] -= model_req[i] * consed_num
            i += 1
        ...
        temp_stock = part_stock                         # reset temp stock
